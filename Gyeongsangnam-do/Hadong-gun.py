...
try:
    ...
    excluded_item = ["심뇌혈관질환예방관리사업", "모바일헬스케어 사업", "건강생활실천사업"]
    
    url = f"{format_url}/01866/01888/01922.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb1 > div.d2 > ul > li.on > div.d3 > ul > li > a"):
        if i.text not in excluded_item:
            id_item = i.get("href").split("01888/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/01866/01888/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for edit_date in soup.select("#chargeUpdateDate"):
