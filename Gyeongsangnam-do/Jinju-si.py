...
try:
    ...
    url = f"{format_url}/05890/05926/06998.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb1 > div.d2 > ul > li.on > div.d3 > ul > li > a"):
        id_item = i.get("href").split("05926/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/05890/05926/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
    ...
    for page_id in page_list:
        page_url = f"{format_url}/05890/05926/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        if elements:
            for element in elements:
                for title in element.select("ul > li.on > a"):
