...
try:
    ...
    url = f"{base_url}?c=HL0503010000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#snb1 > div.d2 > ul > li.on > div.d3 > ul > li.m1 > a"):
        id_item = i.get("href").split("?c=")[1]
    ...
    for page_id in page_list:
        ...
        styles = ["<style> body { background: #fff !important; } .note6  { padding-left: 130px !important; } </style>"]
        ...
        for title in soup.select("#body_title > h1.hb1"):
