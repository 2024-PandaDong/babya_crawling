...
try:
    ...
    url = f"{format_url}/contents.do?mid=0301000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav > ul.snb > li.active > ul.active > li > a"):
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#conts"):
            ...
            for a in content.find_all("a", href=True):
                file_url = a['href']
                if file_url == "#":
                    atchFileId = a.get("onclick").split("'")[1]
                    fileSn = ""
                    if len(a.get("onclick").split("'")) > 3:
                        fileSn = a.get("onclick").split("'")[3]
                    
                    a['href'] = f"{format_url.split('/health')[0]}/common/file/download.do?atchFileId={atchFileId}&fileSn={fileSn}"
