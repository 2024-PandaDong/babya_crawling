...
driver.execute_cdp_cmd('Network.setBlockedURLs', {'urls': ['https://27.101.79.179/?cookie', '*geoje.go.kr/health/index.geoje*']})
...
try:
    ...
    policy_list = ["영양", "임신", "출생", "출산", "산모", "미숙아", "선천성", "영유아", "기저귀", "분유", "냉동난자", "난임", "고위험", "임산부", "산후조리비"]
    
    driver.get(base_url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.gnb-wrap > div.gnb > ul.dl > li.secs > div.sub-gnb > div.sub-gnb-wrap > div.nav > div.bunch5 > div > a"):
        for j in i.select("span"):
            if any(policy in j.get_text() for policy in policy_list):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.contop-info > h3.title"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("div.page-info > div.infos > p.date"):
            for tag in edit_date.find_all("strong"):
                tag.extract()

            data_dict["editDate"] = edit_date.get_text().strip()
        
        for content in soup.select("#contentsBox"):
            for tag in content.find_all(["div", "div", "div"], class_=["contop-info", "sns_bottom", "page-info"]):
                tag.extract()
