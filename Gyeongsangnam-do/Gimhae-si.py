...
try:
    ...
    format_url = base_url.split("/depart")[0]
    ...
    url = f"{format_url}/00976/01553/08040.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#cnb2 > ul.col-3 > li > a"):
        id_item = i.get("href").split("01553/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/00976/01553/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#cnb2 > ul.col-3 > li.on > a"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#body_content"):
            for tag in content.find_all("div", id="cnb2"):
                tag.extract()
