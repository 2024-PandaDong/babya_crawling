...
try:
    ...
    format_url = base_url.split("/specialty")[0]
    ...
    url = f"{format_url}/01889/01926/02153.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb1 > div.d2 > ul > li.on > div.d3 > ul > li > a"):
        id_item = i.get("href").split("01926/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/01889/01926/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("#cnb1")
    ...
    for page_id in page_list:
        page_url = f"{format_url}/01889/01926/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#body_title > h1.h1"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#body_content"):
            for tag in content.find_all("div", id="cnb1"):
                tag.extract()
        ...
        for edit_date in soup.select("#charge1 > dl.update > dd"):
            data_dict["editDate"] = edit_date.get_text().split(" ")[0].replace(".", "-")
