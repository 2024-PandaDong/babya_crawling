...
try:
    ...
    url = f"{format_url}/health/index.goseong?menuCd=DOM_000000604004000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("div.subContentWrap > div.tabmenu > ul.taps-dt4 > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/health/index.goseong?menuCd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.subCnt_top > div.sct_left > h3"):
