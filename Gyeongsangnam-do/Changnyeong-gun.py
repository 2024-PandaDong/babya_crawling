...
try:
    ...
    format_url = base_url.split("/life")[0]
    ...
    url = f"{format_url}/03312/00638/03407.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#cnb1 > ul.col-4 > li > a"):
        id_item = i.get("href").split("00638/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/03312/00638/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
    ...
    for page_id in page_list:
        page_url = f"{format_url}/03312/00638/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
