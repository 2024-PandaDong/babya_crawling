...
try:
    ...
    format_url = base_url.split("/portal")[0]
    ...
    url = f"{format_url}/modules/welfare/info/info.do?cate1=00028&cate2=00029&pageCd=DE0101000000&siteGubun=depart&cpage=1"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("div.pagination > p.pagenav1 > span.pages > span.m > a"):
        if i.get("href") != None:
            id_item = i.get("href").split("&cpage=")[1]
    ...
    for link in link_list:
        driver.get(f"{format_url}/modules/welfare/info/info.do?cate1=00028&cate2=00029&pageCd=DE0101000000&siteGubun=depart&cpage={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        
        for i in soup.select("div.health1welfare1list1 > ul.lst1 > li.li1 > a"):
            id_item = i.get("href").split("?sno=")[1].split("&amode=")[0]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/modules/welfare/info/info.do?sno={page_id}&amode=view&cate1=00028&cate2=00029&pageCd=DE0101000000&siteGubun=depart"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#body_content > div.container > h2.hb1"):
