...
try:
    ...
    url_data = [f"{format_url}/index.ulsan?menuCd=DOM_000000505001001000", f"{format_url}/index.ulsan?menuCd=DOM_000000505010000000"]
    ...
    for url in url_data:
        ...
        for i in soup.select("#lnb > ul > li.depth-1.on > ul.depth3 > li.depth-2 > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/index.ulsan?menuCd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.sub_title > h2"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#printContent"):
        ...
        for edit_date in soup.select("div.cont_footer > div.admin_info > ul > li > span"):
            editDate = edit_date.get_text()
            if re.fullmatch(r"\d{4}-\d{2}-\d{2}", editDate):
