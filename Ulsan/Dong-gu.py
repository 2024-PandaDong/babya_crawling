...
try:
    ...
    url = f"{format_url}/contents/contents.do?mId=2041600"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > ul > li.depth-1.on > ul > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/contents/contents.do?mId={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.tit-wrap > h2"):
            data_dict["title"] = title.get_text().strip()
            
        for content in soup.select("div.in-wrap"):
