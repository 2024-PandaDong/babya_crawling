...
try:
    ...
    format_url = base_url.split("/cmm")[0]
    ...
    url = f"{format_url}/pageCont.do?menuNo=4110100"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#div4110000 > li.dept02 > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/pageCont.do?menuNo={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contents_title > h2"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("div.personinfo > ul > li.last_fix"):
            for tag in edit_date.find_all("span", class_="bold_con"):
                tag.extract()
                
            data_dict["editDate"] = edit_date.get_text().split(": ")[1]
            
        for content in soup.select("#contents_in"):
            for tag in content.find_all("div", class_="personinfo"):
                tag.extract()
            
            for img in content.find_all('img'):
                img_url = img.get("src")
                if img_url:
                    img["src"] = urllib.parse.urljoin(base_url.replace("/cmm/main", ""), img_url)
