...
try:
    ...
    url = f"{format_url}/sub03_04_02.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > li.n5 > ul > li > a"):
        id_item = i.get("href").split("health/")[1].split(";")[0]
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#txt"):
            for tag in content.find_all("div", class_="content-info"):
                tag.extract()
