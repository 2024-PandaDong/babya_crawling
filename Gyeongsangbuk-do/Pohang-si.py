...
try:
    ...
    url = f"{format_url}/contents.do?mid=0308000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#snbWrap > nav > ul.snb > li.active > ul.active > li"):
        is_child_element = i.find("ul")
        if is_child_element:
            for j in soup.select("#snbWrap > nav > ul.snb > li.active > ul.active > li > ul > li > a"):
                id_item = j.get("data-menu-url").split("?mid=")[1]
        ...
        for content in soup.select("#conts"):
            ...
            for a in content.find_all("a", href=True):
                ...
                if file_url == "#":
                    ...
                    a['href'] = f"{format_url.split('/health')[0]}/common/file/download.do?atchFileId={fileid}&file="
