...
try:
    ...
    url = f"{base_url}/main/main.jsp?home_url=health&befor_home_url=health&befor_code=SERVICE_03_02&code=SERVICE_03_01"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#side3m3 > li > a"):
        id_item = i.get("href").split("&befor_code=")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/main/main.jsp?home_url=health&befor_home_url=health{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contents > h1.stitle"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#contents"):
            for tag in content.find_all(["h1", "dl", "p", "div", "div"], class_=["stitle", "location", "cBoth", "box_business", "box_gray"]):
                tag.extract()
