...
try:
    ...
    url = f"{base_url}health.business.info/support/mternal.child/pregnant.management/"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#side-menu > ul > li.current > ul > li > a"):
        id_item = i.get("href").split("/support/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}health.business.info/support/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("#sub-menu")
        
        if elements:
            for element in elements:
                for i in element.select("#sub-menu > li > a"):
                    id_item = i.get("href").split("/support/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}health.business.info/support/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#content > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#content-main"):
            ...
            body_content = f"<body><div id='content'>{content_str}</div></body>"
            
            html_content = f"<!DOCTYPE html><html>{head_content}{body_content}</html>"
            data_dict["content"] = html_content
            
        for edit_date in soup.select("#manager > p.modify-date > span"):
