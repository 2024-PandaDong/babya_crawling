...
try:
    ...
    url = f"{format_url}/page.do?mnu_uid=11488&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#snb > ul.snb > li.on > ul.snb_dept > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.top > div.sub_tit > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#content"):
            for tag in content.find_all("a", class_="go_top"):
                tag.extract()
            
            for tag in content.find_all("div", id="go_birth"):
                tag.extract()
