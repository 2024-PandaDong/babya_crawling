...
try:
    ...
    url_data = [f"{format_url}/contents.do?mid=0406210000", f"{format_url}/contents.do?mid=0408240000"]
    ...
    for url in url_data:
        ...
        for i in soup.select("nav.tab_depth03 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#conts"):
            ...
            for a in content.find_all("a", href=True):
                file_url = a['href']
                if file_url == "#":
                    parts = a.get("onclick").split("'")
                    path = parts[1] if len(parts) > 1 else ""
                    filename = parts[3] if len(parts) > 3 else "" 
                    a['href'] = f"{format_url.split('/health')[0]}/common/file/download.do?atchFileId={path}&fileSn={filename}"
