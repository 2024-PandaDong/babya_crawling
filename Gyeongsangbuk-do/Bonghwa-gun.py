...
try:
    ...
    url = f"{base_url}health.business/baby/"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#side-menu > ul > li > a"):
        if i.text == "모자보건" or i.text == "출산지원" or i.text == "영양플러스":
    ...
    for page_id in page_list:
        page_url = f"{base_url}health.business/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for edit_date in soup.select("#manager > p > span.modify-date"):
