...
try:
    ...
    url = f"{format_url}/contents.do?mId=0302190000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#snbWrap > nav > ul.snb > li.on > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#conts"):
            ...
            for a in content.find_all("a", href=True):
                file_url = a['href']
                if a.get("onclick") and "goFile" in a.get("onclick"):
        ...
        for edit_date in soup.select("div.pageInfo > div.dataOffer > div.date"):
