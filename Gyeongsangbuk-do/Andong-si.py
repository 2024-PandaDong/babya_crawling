...
try:
    ...
    url = f"{format_url}/contents.do?mId=0302000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#left > nav > ul.snb > li > a"):
        if i.text == "모자보건":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.subTopWrap > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#sub_body"):
            ...
            body_content = f"<body><div id='contents'>{content_str}</div></body>"
