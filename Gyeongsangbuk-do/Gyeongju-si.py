...
try:
    ...
    url = f"{format_url}/page.do?mnu_uid=3082&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > ul.dept1 > li > a"):
        if i.text == "모자보건":
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.subTop > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#content"):
            for tag in content.find_all("div", id="pageInfo"):
                tag.extract()
