...
try:
    ...
    url = f"{base_url}/health_work/pregnant_infant/pregnant_care"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("div.section > ul.ui-corner-all > li.sectionA > a"):
        id_item = i.get("href").split("pregnant_infant/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/health_work/pregnant_infant/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = ["<style> body { font-weight: 400 !important; } </style>"]
        ...
        for title in soup.select("#sub_vis > div.inner > h2.tit_subject"):
        ...
        for edit_date in soup.select("#admin_info > dl.admin_info_right > dd.update_cnt"):
