...
try:
    ...
    url = f"{format_url}/page.do?mnu_uid=2643&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#s_nav > ul.depth1 > li > a > span"):
        if i.text == "모자건강":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.depth2 > li > a"):
                for k in j.select("span"):
                    if k.text != "장난감 대여목록" and k.text != "육아용품 대여목록":
    ...
    for link in link_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.sub_tab")
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.top_wrap > h3"):
