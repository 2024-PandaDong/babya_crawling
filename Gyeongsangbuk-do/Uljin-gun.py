...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000904005000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#left_wrap > ul > li.on > ul > li"):
        is_child_element = i.find("ul")
        if is_child_element:
            for j in soup.select("#left_wrap > ul > li.on > ul > li > ul > li > a"):
                id_item = j.get("href").split("?menuCd=")[1]
                current_list.append(id_item)
            
        else:
            for j in soup.select("#left_wrap > ul > li.on > ul > li > a"):
                id_item = j.get("href").split("?menuCd=")[1]
                current_list.append(id_item)
    ...
    for page_id in page_list:
        ...
        styles.append('<link rel="stylesheet" href="./style/Gyeongsangbuk-do/Uljin-gun.style.css"/>')
                
        for title in soup.select("div.s_tit > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("article.s_con"):
            for tag in content.find_all("div", class_="mobile_tab"):
                tag.extract()
