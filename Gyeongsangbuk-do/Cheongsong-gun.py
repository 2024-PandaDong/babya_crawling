...
try:
    ...
    format_url = base_url.split(".web")[0]
    ...
    url = f"{format_url}/00007104/00007153/00007165.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#snb1 > div.d2 > ul > li > a"):
        if i.text == "출산지원" or i.text == "모자보건":
            parent_element = i.find_parent("li")
            for j in parent_element.select("div.d3 > ul > li > a"):
                id_item = j.get("href").split("00007104/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/00007104/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
