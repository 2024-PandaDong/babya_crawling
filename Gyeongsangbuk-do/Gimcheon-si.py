...
try:
    ...
    url = f"{format_url}/contents.do?mId=0301280000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#subNavi > div.wrap > ul.floatL > li > a > span"):
        if i.text == "김천시 출산장려지원 가이드북":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.dept2 > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.subTitle > h3"):
