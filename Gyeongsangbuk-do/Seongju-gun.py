...
try:
    ...
    url_data = [f"{format_url}/page.do?mnu_uid=1445&", f"{format_url}/page.do?mnu_uid=1447&"]
    ...
    for url in url_data:
        ...
        for i in soup.select("#snb_snb01 > div.snb_snb02 > ul.depth01 > li.on"):
            is_child_element = i.find("ul")
            if is_child_element:
                for j in soup.select("#snb_snb01 > div.snb_snb02 > ul.depth01 > li.on > ul > li > a"):
                    id_item = j.get("href").split("?mnu_uid=")[1]
                    current_list.append(id_item)
                
            else:
                for j in soup.select("#snb_snb01 > div.snb_snb02 > ul.depth01 > li.on > a"):
                    id_item = j.get("href").split("?mnu_uid=")[1]
                    current_list.append(id_item)
    ...
    for page_id in page_list:
        ...
        for title in soup.select("section.sub_tit > h4.page_tit"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("section.cont_foot > p.update_info > span"):
            data_dict["editDate"] = edit_date.get_text().split(": ")[1].split(" ")[0]
        
        for content in soup.select("#sub_cont"):
            for tag in content.find_all(["div", "a"], class_=["helth_cont_foot03", "go_cont_link"]):
                tag.extract()
            
            for tag in content.find_all("div", id="cont_top"):
                tag.extract()
