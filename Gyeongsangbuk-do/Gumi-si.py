...
try:
    ...
    url = f"{format_url}/contents.do?mid=0315250100"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav > ul.snb > li.active > ul > li > a"):
        id_item = i.get("data-menu-url").split("?mid=")[1]
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#conts"):
            ...
            for a in content.find_all("a", href=True):
                file_url = a['href']
                if file_url == "#":
                    atchFileIdEncrypted = a.get("onclick").split("'")[1]
                    a['href'] = f"{format_url.split('/health')[0]}/common/file/download.do?atchFileId={atchFileIdEncrypted}&fileSn="
