...
try:
    ...
    format_url = base_url.split("/?page_id=")[0]
    ...
    url = f"{format_url}/?p=1690"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#dc_jqaccordion_widget-59-item > ul.menu > li > a"):
        if i.text == "보건사업안내":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.sub-menu > li.menu-item > a"):
                if j.text == "모자보건" or j.text == "출산장려사업":
                    id_item = j.get("href").split("?p=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/?p={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = ["<style> .rtbs_content { display: block !important; } </style>"]
        ...
        for title in soup.select("div.RT_ttl > h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.post-content"):
            for tag in content.find_all("div", class_="rtbs_menu"):
                tag.extract()
