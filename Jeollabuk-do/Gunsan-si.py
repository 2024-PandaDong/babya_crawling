...
try:
    ...
    url = f"{base_url}/m1574"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > div.lnb > div.active > div.sublnb > div.subname > ul.depth > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.title-wrap > h3"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("div.pageInformation > div.pageInboxw > div.pageInbox-top > div.pageInbox-txt > span"):
            editDate = edit_date.get_text().strip()
            
            if re.match(r"^\d{4}-\d{2}-\d{2}$", editDate):
                data_dict["editDate"] = editDate
        
        for content in soup.select("#containertab"):
            for tag in content.find_all("div", class_="pageInformation"):
                tag.extract()
