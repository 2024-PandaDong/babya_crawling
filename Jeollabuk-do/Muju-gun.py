...
try:
    ...
    url = f"{base_url}?contentUid=ff8080816cf073f2016cfa66e15c09fe"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.tab-menu > div.web_Ty2 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#conHTitle"):
            data_dict["title"] = title.get_text().strip()
        
        for content in soup.select("#contents"):
            for tag in content.find_all(["div", "div"], class_=["tabmenu", "tab-menu"]):
                tag.extract()
