...
try:
    ...
    url = f"{base_url}?contentUid=ff808081898ba9ba018a216a91980624"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#s_wrap > section.left_menu > dl > dd.on > ul > li > a"):
        id_item = i.get("href").split("?contentUid=")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}?contentUid={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#s_wrap > section.left_menu > dl > dd.on > ul > li.on > a"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#main"):
