...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000204004000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.left_menu > ul > li > ul:not(.hide) > li"):
        is_child_element = i.find("ul")
        if is_child_element:
            for j in soup.select("div.left_menu > ul > li > ul:not(.hide) > li > ul > li > a"):
                id_item = j.get("href").split("?menuCd=")[1]
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.s_title > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.s_con"):
            ...
            for a in content.find_all("a", href=True):
                file_url = a['href']
                if file_url == "#map":
                    title = a.get("onclick").split("'")[1]
                    addr = a.get("onclick").split("'")[3]
                    tel = a.get("onclick").split("'")[5]
                    posX = a.get("onclick").split("'")[7]
                    posY = a.get("onclick").split("'")[9]
                    medi_sid = a.get("onclick").split("'")[11]
                    a['onclick'] = ""  # 이상한 창이 띄워지는 버그 떄문에 onclick 삭제
                    a['href'] = f"{base_url}?contentsSid=279&title={title}&addr={addr}&tel={tel}&posX={posX}&posY={posY}&MEDI_SID={medi_sid}"
