...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000602001000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#container > div.layout > aside > ul > li.active > ul > li > a"):
    ...
    for link in link_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.grid-tab-4")
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.bread-cast > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#sub-contents"):
            for tag in content.find_all(["div", "div", "div"], class_=["grid-tab-4", "cclBox", "customerBox"]):
                tag.extract()
