...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000804003000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("section.sub_container > div.s_content > div.s_left > div.s_left > ul > li.on > ul > li > a"):
    ...
    for link in link_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.basic_tab")
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#content"):
            for tag in content.find_all("div", class_="basic_tab"):
                tag.extract()
