...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000603014000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.subNavWrap > ul.subNavBox > li.subNav_List > ul.subNav_inner.active > li"):
        is_child_element = i.find("ul", class_="subNav_inner")
        if is_child_element:
            for j in i.select("ul.subNav_inner > li > ul.subNav_inner > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.sct_left > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#contents"):
            for tag in content.find_all(["div", "div", "div"], class_=["tap-box", "license", "scon_bottom"]):
                tag.extract()
