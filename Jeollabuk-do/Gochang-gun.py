...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000001504008000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("section.sub_container > div.s_content > div.s_left > ul > li.on > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for edit_date in soup.select("div.scon_bottom > div.scon_bcon > div.manager > p"):
