...
try:
    ...
    policy_list = ["영양", "임신", "출생", "출산", "산모", "미숙아", "선천성", "영유아", "기저귀", "분유", "냉동난자", "난임", "고위험", "임산부"]
    
    url = f"{format_url}/depart/index.do?menuUid=ff8080818ec025eb018ec1f5ff880599"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.menu_all > ul.gnb > li.gnb1 > div.box > ul.gnb_2dep > li"):
        is_child_element = i.find("ul")
        if is_child_element:
            for j in i.select("ul > li > a"):
                for k in j.select("span"):
                    if any(policy in k.get_text() for policy in policy_list):
                        id_item = j.get("href").split("?menuUid=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/depart/index.do?menuUid={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.area_pageTitle > h3"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("#satibox > ul > li > span"):
        ...
        for content in soup.select("#content"):
            for tag in content.find_all("aside", id="aside"):
                tag.extract()
