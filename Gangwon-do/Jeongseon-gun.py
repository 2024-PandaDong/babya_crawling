...
try:
    ...
    # 한 페이지에 정책이 다 있음
    current_list.append("motherchild")
    
    page_list = set(current_list) - set(collected_list)

    for page_id in page_list:
        page_url = f"{base_url}/jeongseongun/healthcenter/healthpromotion/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        ...
        for title in soup.select("div.contents > h4.titLv"):
        ...
        for edit_date in soup.select("div.fmSatis-contact > div.modify"):
            for tag in edit_date.find_all(["img", "span"]):
                tag.extract()
