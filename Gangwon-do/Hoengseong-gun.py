...
try:
    ...
    url = f"{format_url}/contents.do?key=2130&"
    driver.get(url)
    time.sleep(2)
    ...
    for link in link_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.cms_tab_box")
        
        if elements:
            for element in elements:
                for i in element.select("div.tab_depth > ul.cms_tab_list > li.cms_tab_item > a.cms_tab_link"):
                    id_item = i.get("href").split("?key=")[1]
    ...
    for page_id in page_list:
        ...
        for title in soup.select("header.sub_head > div.top_box > div.title_box > h2.title > span > em"):
