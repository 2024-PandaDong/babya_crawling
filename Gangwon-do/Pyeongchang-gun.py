...
try:
    ...
    url_data = [f"{base_url}/service/service-health", f"{base_url}/service/service-nutrition"]
      
    for url in url_data:
        driver.get(url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("ul.pageTab")
        
        if elements:
            for element in elements:
                for i in element.select("ul.pageTab > li > a"):
                    id_item = i.get("href").split("service/")[1]
                    current_list.append(id_item)
                    
        else:
            current_list.append(url.split("service/")[1])
    
    
    page_list = set(current_list) - set(collected_list)

    for page_id in page_list:
        page_url = f"{base_url}/service/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.sub_view_area > div.subcontainer_tit > h2 > strong.tit"):
        ...
        for edit_date in soup.select("div.satisfaction > div.chargewrapper > ul > li"):
            for tag in edit_date.find_all("span"):
                tag.extract()
                
            editDate = edit_date.get_text()
            if re.match(r"^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$", editDate):
                data_dict["editDate"] = editDate.split(" ")[0]
