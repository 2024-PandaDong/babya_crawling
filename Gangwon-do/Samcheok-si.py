...
try:
    ...
    url = f"{format_url}/specialty/00466/01253.web"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#cnb1 > ul.col-5 > li > a"):
        id_item = i.get("href").split("00466/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/specialty/00466/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#cnb1 > ul.col-5 > li.on > a"):
