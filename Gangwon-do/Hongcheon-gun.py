...
try:
    ...
    url = f"{format_url}/selectBbsNttList.do?key=881&bbsNo=122&searchCtgry=&pageUnit=10&searchCnd=all&searchKrwd=&pageIndex=1"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.p-pagination > div.p-page > span > a"):
        if i.get_text().strip() != "다음 페이지":
            id_item = i.get("href").split("&pageUnit=")[1]
            link_list.append(id_item)
    
    for link in link_list:
        url = f"{format_url}/selectBbsNttList.do?key=881&bbsNo=122&searchCtgry=&pageUnit={link}"
        driver.get(url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        
        for i in soup.select("table.p-table > tbody.text_center > tr > td.p-subject > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/selectBbsNttView.do?key={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("tbody.p-table--th-left > tr.p-table__subject > td > span.p-table__subject_text"):
            data_dict["title"] = title.get_text()

        for content in soup.select("table.p-table"):
        ...
        for edit_date in soup.select("footer.sub_foot > div.satisfaction > div.manager_info > ul.clearfix > li"):
