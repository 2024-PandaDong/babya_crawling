...
try:
    ...
    url_data = [f"{format_url}/new-welfare/life/pregnancy-birth/encouragement-policy/", f"{format_url}/new-welfare/life/infant/childcare-subsidy/"]
    ...
    for url in url_data:
        ...
        for i in soup.select("#main > div.aside > ul.nav > li.active > ul > li > a"):
            if i.get("target") != "_blank" and i.get_text() != " 보육사업안내":
                id_item = i.get("href").split("life/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/new-welfare/life/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#print-wrap > div.sub-top > h2"):
            data_dict["title"] = title.get_text()

        for content in soup.select("#print-wrap > section.con-area"):
        ...
        for edit_date in soup.select("div.common-area > div.right-area > div.update > p > span.update-day"):
