...
try:
    ...
    url = f"{format_url}/sub03_02_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > li.n2 > ul.depth3_ul > li > a"):
    ...
    for link in link_list:
        ...
        if elements:
            for element in elements:
                for i in element.select("ul.tab-ul > li > a"):
                    id_item = i.get("href").split("health/")[1]
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#txt"):
            for tag in content.find_all("div", id="tab_moType1"):
                tag.extract()
