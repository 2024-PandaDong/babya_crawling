...
try:
    ...
    url = f"{format_url}/sub04_05_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > li.th2 > a.link_2th_ov"):
        if i.text == "모자건강":
            parent_div = i.find_parent('li')
            for j in parent_div.select("ul.th3 > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#navigate > h1"):
        ...
        for edit_date in soup.select("div.charge > ul.admin_info > li.charger4"):
