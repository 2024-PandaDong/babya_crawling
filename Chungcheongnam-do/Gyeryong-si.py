...
try:
    ...
    url = f"{base_url}html/sub03/03030101.html"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > li.on > ul.depth3_ul > li.on > ul.depth4_ul > li > a"):
        id_item = i.get("href").split("sub03/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}html/sub03/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
