...
try:
    ...
    url = f"{format_url}/sub04_04_13.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnbwrap > ul.lm_2th > li > a"):
        if i.text == "모자보건":
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#location > h3"):
            data_dict["title"] = title.get_text().strip()
            
        for content in soup.select("#contents"):
            for tag in content.find_all("div", class_="charge"):
                tag.extract()
