...
try:
    ...
    url = f"{base_url}work/work_040901.html"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#submenu10 > ul.lm_3th > li > a"):
        if i.text != "모유수유실천사업":
            id_item = i.get("href").split("work/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}work/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contTi"):
