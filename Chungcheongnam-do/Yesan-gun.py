...
try:
    ...
    url = f"{format_url}/sub03_06_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb_layout > ul.depth2_ul > li.n6.on > ul.depth3_ul > li > a"):
    ...
    for page_id in page_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        cont_num = len(soup.select("ul.ui-accordion--faq > li.accordion__item"))
        ...
        if cont_num:
            for idx in range(cont_num):
                ...
                styles = ['<style> [style*="display: none;"] { display: block !important; } </style>']
                ...
                soup_list = soup.select("ul.ui-accordion--faq > li.accordion__item")
                main_soup = soup_list[idx]
                
                # title
                for title in main_soup.select("div.accordion__header > a.accordion__button > span.accordion__title"):
                    data_dict["title"] = title.get_text()
                    
                # content
                for content in main_soup.select(f"#tab{idx+1}-1"):
                    for tag in content.find_all("div", class_=["Con-area", "MAT40"]):
                        tag.extract()
