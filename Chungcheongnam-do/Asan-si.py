...
try:
    ...
    url = f"{base_url}index.php?m_cd=308"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#acdn_menu > ul.acdnTop > li.on.acdnSeparator > ul.acdnSub > li > a"):
        id_item = i.get("href").split("?m_cd=")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}index.php?m_cd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for link in soup.select("html > head > link"):
            if link.get("rel")[0] == "stylesheet":
                ...
                if "sub03" not in link_url:
                    link["href"] = urllib.parse.urljoin(base_url, link_url)
                    styles.append(str(link))

        for title in soup.select("div.sub_container > h1"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("div.sub_con"):
            for tag in content.find_all(["div", "div"], class_=["tab_zone", "permission_zone"]):
                tag.extract()
