...
try:
    ...
    url = f"{format_url}/sub03_11_01_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lm > li > a"):
        if i.text == "모자보건사업":
            parent_div = i.find_parent('li')
            for j in parent_div.select("ul.lm_3th > li > a"):
    ...
    for page_id in page_list:
        ...
        styles = []
        subPage_list = []
        elements = soup.select("ul.basic_tab > li:not(.on) > a")
        ...
        for title in soup.select("div.content_title > h3 > span"):
            data_dict["title"] = title.get_text().strip()
            
        for content in soup.select("#txt"):
            for tag in content.find_all("ul", class_="basic_tab"):
                tag.extract()
        ...
        for edit_date in soup.select("div.charge > dl.modDate > dd"):
        ...
        for subPage_id in subPage_list:
            ...
            for subTitle in subSoup.select("div.content_title > h3 > span"):
                data_dict["title"] = subTitle.get_text().strip()
                
            for subContent in subSoup.select("#txt"):
                for tag in subContent.find_all("ul", class_="basic_tab"):
                    tag.extract()
            ...
            for subEdit_date in subSoup.select("div.charge > dl.modDate > dd"):
