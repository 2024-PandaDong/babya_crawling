...
try:
    ...
    format_url = base_url.split("/hvh")[0]
    ...
    url = f"{format_url}/mch/1.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#contents_area > div.depth4_tabmenu > ul.cell10 > li > a"):
        if i.text != "어르신 폐렴구균 예방접종사업":
            id_item = i.get("href").split("mch/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/mch/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contents > h3.title04"):
