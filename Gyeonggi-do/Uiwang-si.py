...
try:
    ...
    url = f"{format_url}/UWHEAL0403"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > ul.lnb > li.active > ul.lnb2Wrap > li > a"):
        id_item = i.get("href").split("heal/")[1]
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#main > div.headWrap > h2.h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#main > div.spotWrap"):
            for tag in content.find_all("div", class_="tab_list"):
                tag.extract()
        ...
        for edit_date in soup.select("section.bannerWrap > div.box > div.top > span.date"):
