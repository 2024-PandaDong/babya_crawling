...
try:
    ...
    url = f"{format_url}/homepage/menu/viewMenu?menuid=161004004001001"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb_nav > ul.list > li.on > ul.dep_list > li > a"):
        id_item = i.get("href").split("?menuid=")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/homepage/menu/viewMenu?menuid={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("nav.tab_ty")
        
        if elements:
            for element in elements:
                for i in element.select("ul.tab_list > li > a"):
                    id_item = i.get("href").split("?menuid=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/homepage/menu/viewMenu?menuid={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.page_tit > h3.title"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.sub_contents"):
        ...
        for edit_date in soup.select("#page_info > div.box > dl.info_update > dd"):
