...
try:
    ...
    url = f"{format_url}/common/cntnts/selectContents.do?cntnts_id=C0001657"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.menu > div.depth1 > ul.depth1_list > li.actived > div > ul > li > a"):
        id_item = i.get("href").split("?cntnts_id=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/common/cntnts/selectContents.do?cntnts_id={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
