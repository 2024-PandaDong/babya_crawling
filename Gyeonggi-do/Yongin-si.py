...
try:
    ...
    for page_id in page_list:
        page_url = f"{format_url}/home/health/healthSvc/mcHealth/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contents > div.h3_box > h3"):
            data_dict["title"] = title.get_text().strip()
        
        for content in soup.select("#contents > div.cont_box"):
