...
try:
    ...
    url = f"{format_url}/contents.do?key=713"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#side > div.side_menu > nav.menu > div.depth1 > ul.depth1_list > li.active > div > ul > li"):
        elements = i.select("div.depth3")
        if elements:
            for element in elements:
                for j in element.select("div.depth3 > ul.depth3_list > li.depth3_item > a"):
                    id_item = j.get("href").split("?key=")[1]
                    current_list.append(id_item)
                    
        else:
            for j in i.select("a.depth2_text"):
                id_item = j.get("href").split("?key=")[1]
                current_list.append(id_item)
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#contents"):
            for tag in content.find_all(["div", "div"], class_=["emphasis", "satisfaction"]):
                tag.extract()
