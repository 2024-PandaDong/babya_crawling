...
try:
    ...
    url = f"{format_url}/contents.do?mId=0304080100"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#container > section.snb_wrap > nav > ul.snb > li > a > span"):
        if i.text == "모자보건사업":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.snb > li > ul > li > a"):
    ...
    for link in link_list:
        ...
        if elements:
            for element in elements:
                for i in element.select("nav.tab_depth04 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#conts"):
            for tag in content.find_all("div", class_="btnboxTop"):
                tag.extract()
            
            for tag in content.find_all("div", id="body_tap_wrap"):
                tag.extract() # 추후 마이그레이션 할 예정
            ...
            for a in content.find_all("a", href=True):
                ...
                if file_url == "#":
                    ...
                    a['href'] = f"{format_url.split('/health')[0]}/FileDown_direct.do?path={path}&file={filename}"
