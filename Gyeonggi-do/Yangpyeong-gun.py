...
try:
    ...
    url = f"{format_url}/contents.do?key=1676"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.sub_menu > ul.sm_depth2 > li.active > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        styles = []
        menu_element = soup.select("div.sm_depth4")
        ...
        for title in soup.select("nav.sub_menu > ul.sm_depth2 > li.active > ul.current > li.current > a.current > span"):
        ...
        for content in soup.select("#contents"):        
            ...
            if menu_element:
                for element in menu_element:
                    content_add = "<div>\n"
                    for menu in element.select("div.submenu-wrap > ul.clearfix > li > a"):
                        id_item = menu.get("href").split("?key=")[1]
                        menu_url = f"{format_url}/contents.do?key={id_item}"
                        driver.get(menu_url)
                        time.sleep(2)
                        soup = BeautifulSoup(driver.page_source, 'html.parser')
                        ...
                        for menu_content in soup.select("#contents"):
                            content_add += re.sub(r'[\s\u00A0-\u00FF]+', " ", str(menu_content)) + "\n"
        ...
        for edit_date in soup.select("aside.content-information > section.manager > ul > li.date"):
            for tag in edit_date.select("em"):
                tag.extract()
