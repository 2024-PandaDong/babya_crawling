...
try:
    ...
    url = f"{format_url}/contents.do?key=948"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.menu > div.depth1 > ul.depth1_list > li.active > div > ul > li > div > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for edit_date in soup.select("footer.satisfy > div.manager_info > ul.clearfix > li.n3"):
            for tag in edit_date.select("span"):
                tag.extract()
                
            data_dict["editDate"] = edit_date.get_text().strip().replace(".", "-")
        
        for content in soup.select("#contents"):
            for tag in content.find_all("footer", class_="satisfy"):
                tag.extract()
