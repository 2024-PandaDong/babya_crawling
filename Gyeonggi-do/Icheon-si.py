...
try:
    ...
    url = f"{format_url}/depart/contents.do?mid=0604070100"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#content > nav.tab_depth04 > ul.list03 > li > a"):
    ...
    for link in link_list:
        ...
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("#content > div.tab_depth05")
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#titWrap > h3 > strong"):
