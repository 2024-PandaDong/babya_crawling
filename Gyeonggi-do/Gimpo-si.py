...
try:
    ...
    url = f"{format_url}/contents.do?key=2187"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.menu > div.depth1 > ul.depth1_list > li.depth1_item > a"):
        parent_element = i.find_parent("li")
        if i.text != "예방접종":
            for j in parent_element.select("div.depth2 > ul.depth2_list > li.depth2_item > a"):
                if j.text != "출산예정일 계산":
    ...
    for page_id in page_list:
        ...
        for title in soup.select("header.sub_head > div.sub_head_wrap > div.clearfix > div.sub_title > h2"):
        ...
        for content in soup.select("#contents"):
            for tag in content.find_all("footer", class_="sub_foot"):
                tag.extract()
