...
try:
    ...
    url = f"{format_url}/depart/contents.do?mId=0805060100"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav > ul.snb > li > a > span"):
        if i.text == "보건사업":
            parent_li = i.find_parent("li")
            for j in parent_li.select("ul > li > a > span"):
                if j.text == "모자보건" or j.text == "임신출산지원":
                    parent_a = j.find_parent("a")
                    id_item = parent_a.get("href").split("?mId=")[1]
                    link_list.append(id_item)
                    
    for link in link_list:
        driver.get(f"{format_url}/depart/contents.do?mId={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("nav.tabDepth04")
    ...
    for page_id in page_list:
        page_url = f"{format_url}/depart/contents.do?mId={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for edit_date in soup.select("div.pageInfo > div.dataOffer > dl.date > dd"):
            data_dict["editDate"] = edit_date.get_text().strip()
        
        for content in soup.select("#conts"):
            for tag in content.find_all("div", class_="pageInfo"):
                tag.extract()
            ...
            for a in content.find_all("a", href=True):
                ...
                if file_url == "#":
                    ...
                    a['href'] = f"{format_url}/file/direct/download.do?path={path}&filename={filename}"
