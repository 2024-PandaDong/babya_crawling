...
try:
    ...
    url_data = [f"{format_url}/contents.do?key=1259", f"{format_url}/contents.do?key=3551"]
    ...
    for url in url_data:
        ...
        for i in soup.select("nav.menu > div.depth1 > ul.depth1_list > li.depth1_item.active > div > ul > li > a"):
    ...
    for link in link_list:
        ...
        if elements:
            for element in elements:
                for i in element.select("div.tab_menu > div.tab_panel > ul > li > a"):
                    for j in i.select("span.tablebox > span"):
