...
try:
    ...
    url = f"{format_url}/business/region/motherchild/motherchild04.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.sub_tab04 > ul > li > a"):
        id_item = i.get("href").split("motherchild/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/business/region/motherchild/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for style in soup.find_all("style"):
            style_tag = f"<style>{style.get_text()}</style>"
            styles.append(style_tag)
                
        for title in soup.select("div.sub_tab04 > ul > li.sub_tab04_on > a"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.sub_rgtCon"):
            for tag in content.find_all(["h3", "div"], class_=["content_title", "sub_tab04"]):
                tag.extract()
