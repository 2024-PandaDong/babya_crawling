...
try:
    ...
    url = f"{format_url}/contents.do?key=1008817"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#side > div.side_menu > nav.menu > div.depth1 > ul.depth1_list > li.active > div > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("header.sub_head > div.sub_info > div.sub_title > h2 > strong"):
