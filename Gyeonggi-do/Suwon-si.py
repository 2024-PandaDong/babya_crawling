...
try:
    ...
    url = f"{format_url}/sub.asp?page_code=sub05020501"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#side3m9 > li > a"):
        id_item = i.get("href").split("?page_code=")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/sub.asp?page_code={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("article.boxTab")
        
        if elements:
            for element in elements:
                for i in element.select("ul > li > a"):
                    for j in i.select("span"):
                        if j.text != "출산예정일 확인하기":
                            id_item = i.get("href").split("?page_code=")[1]
    ...
    for tab_id in tab_list:
        driver.get(f"{format_url}/sub.asp?page_code={tab_id}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.tab_area")
    ...
    for page_id in page_list:
        page_url = f"{format_url}/sub.asp?page_code={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#body_head_outer > div.body_head > div.body_title > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.content_area"):
            for tag in content.find_all(["article", "div", "ul"], class_=["boxTab", "tab_area", "call"]):
                tag.extract()
