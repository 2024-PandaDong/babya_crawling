...
try:
    ...
    url = f"{format_url}/helthBsns/mhrhlHelth/pwnmManage.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > div.lnb > ul > li.lnb_on > ul.lnb_2dep > li > a"):
        id_item = i.get("href").split("mhrhlHelth/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/helthBsns/mhrhlHelth/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = ["<style> html { display: flex; justify-content: flex-start; } </style>"]
        ...
        for title in soup.select("#sub_contents > h4.title_ty01"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#sub_contents"):
            for tag in content.find_all("h4", class_="title_ty01"):
                tag.extract()
                
            for tag in content.find_all("div", id="share"):
                tag.extract()
