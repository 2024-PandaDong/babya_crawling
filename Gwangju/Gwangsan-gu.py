...
try:
    ...
    url = f"{base_url}contentsView.do?pageId=health46"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.left_menu > ul > li.active > ul > li > a"):
        id_item = i.get("href").split("?pageId=")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}contentsView.do?pageId={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.tabs")
    ...
    for page_id in page_list:
        page_url = f"{base_url}contentsView.do?pageId={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = ["<style>*::after {content: none !important;}</style>"]
        ...
        for title in soup.select("div.content_top > h3"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("div.content"):
            for tag in content.find_all(["div", "div"], class_=["content_top", "tabs"]):
                tag.extract()
