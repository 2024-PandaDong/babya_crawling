...
try:
    ...
    url = f"{format_url}/contents.do?key=4568&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("nav.menu.init > div.depth1 > ul.depth1_list > li.active > div.depth2 > ul.depth2_list > li"):
        elements = i.select("div.depth.depth3")
        
        if elements:
            for element in elements:
                for j in element.select("ul.depth3_list > li.depth3_item > a.depth3_text"):
                    id_item = j.get("href").split("?key=")[1]
                    current_list.append(id_item)
                    
        else:
            for j in i.select("a.depth_text.depth2_text"):
                href = j.get("href")
                if "?key=" in href:
                    id_item = href.split("?key=")[1]
                    current_list.append(id_item)
