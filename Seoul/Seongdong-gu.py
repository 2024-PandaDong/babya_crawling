...
try:
    ...
    # https://www.sd.go.kr/health/index.do
    ...
    url = f"{format_url}/contents.do?key=2305&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, "html.parser")

    for i in soup.select("ul.depth_list.depth1_list > li.active > div.depth.depth2 > ul > li"):
        is_chlid_element = i.find("div")
        if is_chlid_element:
            for j in i.select("div.depth.depth3 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("main.colgroup > article > header.sub_head > div.sub_info > div.sub_title > h2"):
        ...
        for edit_date in soup.select("footer.satisfaction > div.manager_info.clearfix > ul > li > span"):
            if edit_date.get_text() == "최종수정일":
                parent_element = edit_date.find_parent('li')
                edit_date.extract()
                data_dict["editDate"] = parent_element.get_text().strip().replace(".", "-")
