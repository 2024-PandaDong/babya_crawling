...
try:
    ...
    url_data = [f"{format_url}/contents.do?key=1320&", f"{format_url}/contents.do?key=1310&", f"{format_url}/contents.do?key=1357&"]
    ...
    for url in url_data:
        ...
        for i in soup.select("div.depth.depth2 > ul.depth_list.depth2_list > li.active > div.depth.depth3 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("header.sub_head > div.sub_info > div.sub_title > h2"):
        ...
        for edit_date in soup.select("footer.sub_foot > div.satisfaction > div.manager_info > ul.clearfix > li > span"):
            if edit_date.get_text() == "콘텐츠수정일":
                parent_element = edit_date.find_parent("li")
                edit_date.extract()
                
                data_dict["editDate"] = parent_element.get_text().strip().rstrip(".").replace(".", "-")
