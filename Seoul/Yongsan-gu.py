...
try:
    ...
    url = f"{base_url}/main/contents.do?menuNo=300098"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("ul > li.on > ul > li > a"):
        id_item = i.get("href").split("menuNo=")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}/main/contents.do?menuNo={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("ul.radius-lg")
        
        if elements:
            for element in elements:
                for i in element.select("ul.radius-lg > li > a"):
                    id_item = i.get("href").split("menuNo=")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/main/contents.do?menuNo={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
