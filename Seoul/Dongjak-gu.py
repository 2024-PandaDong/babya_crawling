...
try:
    ...
    format_url = base_url.split("/main.do")[0]
    ...
    url = f"{format_url}/contents.do?menuNo=300342"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("div.left-area > ul.left-mdp1 > li.on > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.content-area > div.title-zone > h2.pg-tit"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#contentDiv"):  
        ...
        for edit_date in soup.select("div.page-inf > div.up > dl > dd"):
            date_str = edit_date.get_text(strip=True)
