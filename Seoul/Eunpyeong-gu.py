...
try:
    ...
    url = f"{format_url}/contents.do?key=1573"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, "html.parser")

    for i in soup.select("li.active > div.depth.depth2 > ul.depth_list.depth2_list > li"):
        is_child_element = i.find("div", class_="depth3")
        if is_child_element:
    ...
    for page_id in page_list:
        ...
        for edit_date in soup.select("footer.satisfaction > div > div.box_date > p > span.caution_date"):
