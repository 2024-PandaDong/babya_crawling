...
try:
    ...
    url = f"{format_url}/contents.do?key=4930&"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("ul.depth1_list > li.active > div.depth2 > ul.depth2_list > li > div.depth3 > ul > li > a"):
    ...
    for link in link_list:
        ...
        if elements:
            for element in elements:
                for i in element.select("ul.clearfix > li > a"):
                    id_item = i.get("href").split("?key=")[1]
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#contents"):
            for tag in content.find_all('ul', class_='clearfix'):
                tag.extract()
