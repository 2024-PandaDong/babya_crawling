...
try:
    ...
    format_url = base_url.split("/gwanak")[0]
    ...
    url = f"{format_url}/health/05/10502010200002022080207.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("nav > ul.dep1 > li.open > ul.dep2 > li"):
    ...
    for page_id in page_list:
        ...
        styles = ["<style>#contents {text-align: start;}</style>"]
        ...
        for edit_date in soup.select("div.contents-mnt > ul > li.update > em"):
