...
try:
    ...
    url = f"{format_url}/contents.do?menuNo=300080"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, "html.parser")

    for i in soup.select("li.on > div > ul > li > a"):
        id_item = i.get("href").split("menuNo=")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{format_url}/contents.do?menuNo={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("main > div.tab.toa > div.tab-list")
        
        if elements:
            for element in elements:
                for i in element.select("div > a"):
                    id_item = i.get("href").split("menuNo=")[1]
    ...
    for page_id in page_list:
        ...
        for title in soup.select("main > div.hgroup > h1"):
        ...
        for edit_date in soup.select("dd#cmi-updtDt"):
