...
try:
    ...
    format_url = base_url.split("/index.do")[0]
    ...
    url = f"{format_url}/healthIncrz/healthIncrz1/healthIncrz1_01.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, "html.parser")

    for i in soup.select("#snb > li.collapse.on > ul > li > a"):
        id_item = i.get("href").split("healthIncrz1_")[1].split(".jsp")[0]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/healthIncrz/healthIncrz1/healthIncrz1_{page_id}.jsp"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#content > div.content-header > h2.content-title"):
            data_dict["title"] = ' '.join(title.get_text().split())
            
        for content in soup.select("#content > div.content-body"):
