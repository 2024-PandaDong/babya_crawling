...
try:
    ...
    url = f"{format_url}/viewTnHealthLfeCycleBsnsU.do?key=20540&no=66"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.side_menu > div.depth1 > ul.depth1_list > li.active > div > ul > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/viewTnHealthLfeCycleBsnsU.do?key={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for content in soup.select("#contents"):
            for tag in content.find_all("div", class_="margin_t_30"):
                tag.extract()
