...
try:
    ...
    url_data = [f"{base_url}contents.do?key=1735", f"{base_url}contents.do?key=1741"]
    ...
    for url in url_data:
        ...
        for i in soup.select("#side > nav.side_menu > ul > li.on > ul > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{base_url}contents.do?key={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
