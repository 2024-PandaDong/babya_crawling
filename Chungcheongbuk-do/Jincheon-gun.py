...
try:
    ...
    url = f"{format_url}/sub.do?menukey=698"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > div.snb-wrap > div.side-menu > ul.side-menu-wrap > li.js-open-d > ul > li"):
        is_child_element = i.find("ul", class_="side-3dep-wrap")
        
        if is_child_element:
            for j in i.select("ul.side-3dep-wrap > li.side-3dep > a"):
                id_item = j.get("href").split("?menukey=")[1]
                current_list.append(id_item)
            
        else:
            for j in i.select("li.side-2dep > a"):
                id_item = j.get("href").split("?menukey=")[1]
                current_list.append(id_item)
    ...
    for page_id in page_list:
        page_url = f"{format_url}/sub.do?menukey={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.sub_title-wrap > div.sub_title > h2"):
        ...
        for content in soup.select("#contents"):
            ...
            body_content = f"<body id='sub'>{content_str}</body>"
