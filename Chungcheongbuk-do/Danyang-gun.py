...
try:
    ...
    url = f"{format_url}/health/184"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    current_list.append("184") # 메뉴바에 없는 임산부 정책이 모여있음
    
    for i in soup.select("#menu_submenu > li.cd1 > div.lb1 > ul.ld2 > li.open > div > ul > li"):
        is_child_element = i.find("div", class_="lb3")
        
        if is_child_element:
            for j in i.select("div.lb3 > ul.ld4 > li.cd4 > a"):
                id_item = j.get("href").split("health/")[1]
                current_list.append(id_item)
            
        else:
            for j in i.select("li.cd3 > a.l3"):
                id_item = j.get("href").split("health/")[1]
                current_list.append(id_item)
    ...
    for page_id in page_list:
        page_url = f"{format_url}/health/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#section > section > header > h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#contents"):
            for tag in content.find_all("div", class_="doc_license"):
                tag.extract()
