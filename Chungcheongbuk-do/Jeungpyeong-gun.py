...
try:
    ...
    url = f"{format_url}/sub04_09_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb_layout > ul.depth2_ul > li.on > ul.depth3_ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#lnb_layout > ul.depth2_ul > li.on > ul.depth3_ul > li.on > a"):
        ...
        for edit_date in soup.select("#content > div.namebox > div.charge > dl.modDate > dd"):
