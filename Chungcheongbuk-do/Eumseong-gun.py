...
try:
    ...
    url_data = [f"{format_url}/contents.do?key=1868", f"{format_url}/contents.do?key=1933"]
    ...
    for url in url_data:
        ...
        for i in soup.select("div.tab_box > div.tab_wrap > ul.tab_list > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("header.sub_head > div.sub_top > div.sub_title > h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#contents"):
            for tag in content.find_all("div", class_="tab_box"):
                tag.extract()
        ...
        for edit_date in soup.select("header.sub_head > div.manager_box > div.manager_info > div.final > span.txt"):
