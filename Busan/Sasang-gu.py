...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000404003001001"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > li.has-sub.open > ul > li"):
    ...
    for page_id in page_list:
        ...
        for content in soup.select("#content"):
            for tag in content.find_all(['div', 'h1'], class_=['location', 'title']):
                tag.extract()
                
            for tag in content.find_all(['ul', 'div'], id=['sns_send', 'change']):
                tag.extract()
