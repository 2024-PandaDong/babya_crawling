...
try:
    ...
    url = f"{format_url}/contents.do?mId=0304010000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#li_depth2_0304 > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#sub_body"):
            data_dict["title"] = title.find("h4").get_text()
            
        for content in soup.select("#ctn"):
            for tag in content.find_all(['div', 'div'], class_=['open_license', 'satisfaction']):
                tag.extract()
