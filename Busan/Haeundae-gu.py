...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000804001000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("section.sidebar > ul.sidebar-menu > li.active > ul.treeview-menu > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#tit_cont"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#font_size"):      
        ...
        for edit_date in soup.select("div.satisfaction > div.manager_info > ul.clearfix > li > span.text"):
