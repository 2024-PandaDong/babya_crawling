...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000001504007001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > li.has-sub.open > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.sub_visual > div.content_box > h1.title"):
        ...
        for content in soup.select("#content"):
            for tag in content.find_all('div', class_='nuri_wrap'):
                tag.extract()
