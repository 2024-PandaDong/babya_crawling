...
try:
    ...
    format_url = base_url.split("?menuCd=")[0]
    ...
    url = f"{format_url}?menuCd=DOM_000000904008001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > li > a.on"):
        parent_div = i.find_parent('li')
        for j in parent_div.select("ul.depth3 > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}?menuCd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#skip_wrap > div.contents > h3"):
            data_dict["title"] = title.get_text()
            
            title.extract()
            
        for content in soup.select("div.contents"):
            for tag in content.find_all(['div', 'div', 'div', 'div'], ['subVisual', 'location', 'sns', 'sub_area']):
                tag.extract()
