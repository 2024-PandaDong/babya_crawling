...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000402001001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.nav > div.nav-con > div.depth.has-sub > a > span"):
        if i.text == "모자보건":
            parent_div = i.find_parent('div')
            for j in parent_div.select("ul.depth3-1 > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.cont-top > div.cont_tit > h2"):
        ...
        for edit_date in soup.select("div.page-info > div.infos > p.date"):
            for tag in edit_date.select("strong"):
                tag.extract()
