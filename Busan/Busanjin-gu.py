...
try:
    ...
    url = f"{format_url}/index.busanjin?menuCd=DOM_000000504002002000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lm02 > ul.depth3 > li.lm_l3 > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/index.busanjin?menuCd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.contents > div.contop-info > h3.title"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#sub_contentnw"):
            for tag in content.find_all('div', 'taps-fuldown'):
                tag.extract()
