...
try:
    ...
    url = f"{base_url}?menuCd=DOM_000000404007001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("li#lm06 > ul.depth3 > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.cont-top > div.cont-tit > h2"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#contents > div.content-inner"):
        ...
        for edit_date in soup.select("div.depart-satisfy > div.depart-wrap > p.recent > span"):
