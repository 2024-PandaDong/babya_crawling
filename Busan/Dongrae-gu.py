...
try:
    ...
    url = f"{format_url}/index.dongnae?menuCd=DOM_000000803005000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("nav.sub_menu > ul.sm_2th > li.on > ul.sm_3th > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{format_url}/index.dongnae?menuCd={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#accessibility_mark > h2"):
