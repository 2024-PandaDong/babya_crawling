...
try:
    ...
    format_url = base_url.replace("/health", "")
    ...
    url = f"{format_url}?menuCd=DOM_000000503003001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lnb > li > a"):
        if i.text == "모자보건사업":
            parent_div = i.find_parent('li')
            for j in parent_div.select("#lnb > li > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.subconbox > h1.title"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("div.subconbox"):
