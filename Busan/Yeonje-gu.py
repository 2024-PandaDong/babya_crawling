...
try:
    ...
    url = f"{format_url}/guide/siteMap.do?mId=0800000000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#sitemap > div.amThum > h2.site01 > span"):
        if i.text == "가족보건":
            parent_div = i.find_parent('div', class_='amThum')
            for j in parent_div.select("div.sitemap_grep > ul.sitemap_list > li"):
                is_child_element = j.find("ul")
                if is_child_element:
                    for k in j.select("ul.sitemap_list > li > ul > li > a"):
                        id_item = k.get("href").split("?mId=")[1]
    ...
    for page_id in page_list:
        ...
        for title in soup.select("#content > div.cont-top > div.cont-tit > h2"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("#conts"):
            for tag in content.find_all('div', id='nuri'):
                tag.extract()
        ...
        for edit_date in soup.select("div.depart-satisfy > div.depart-wrap > p.recent"):
            data_dict["editDate"] = ' '.join(edit_date.get_text().split()).split(": ")[1]
