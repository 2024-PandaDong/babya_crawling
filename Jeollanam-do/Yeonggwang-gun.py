...
try:
    ...
    format_url = base_url.split("/?site")[0]
    ...
    current_list.append("headquarter_new&mn=9359")

    page_list = set(current_list) - set(collected_list)

    for page_id in page_list:
        page_url = f"{format_url}/subpage/?site={page_id}"
        driver.get(page_url)
        time.sleep(2)
        ...
        styles = ["<style> .sub_box { display: block !important; } </style>"]
        ...
        for title in soup.select("#contents_top > div.title > h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#contents"):
            for tag in content.find_all(["div", "div"], class_=["tab_type1", "tab_type2"]):
                tag.extract()
        ...
        for edit_date in soup.select("#department_info > ul.dptinfo > li.refresh"):
