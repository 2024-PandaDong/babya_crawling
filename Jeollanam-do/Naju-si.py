...
try:
    ...
    url = f"{base_url}business/maternal/before_pregnancy"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li.on > ul > li > a"):
        id_item = i.get("href").split("maternal/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}business/maternal/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.section_wrap > ul.depth4")
        
        if elements:
            for element in elements:
                for i in element.select("li > a"):
                    id_item = i.get("href").split("maternal/")[1]
                    tab_list.append(id_item)
                    
        else:
            tab_list.append(link)
            
    for tab_id in tab_list:
        driver.get(f"{base_url}business/maternal/{tab_id}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.section_wrap > ul.depth5")
        ...
        if elements:
            ...
            current_list.append(tab_id)
    

    page_list = set(current_list) - set(collected_list)

    for page_id in page_list:
        page_url = f"{base_url}business/maternal/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = ["<style> * { font-weight: 500 !important; } </style>"]
        elements = soup.select("div.section_wrap > ul.depth5")
        ...
        if elements:
            for element in elements:
                for title in element.select("li.on > a"):
                    data_dict["title"] = title.get_text()
            
        else:   
            for title in soup.select("#right > div.cont_titlebox > h3"):
                data_dict["title"] = title.get_text()
