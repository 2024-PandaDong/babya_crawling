...
try:
    ...
    item_data = ["영유아", "보육아동", "임산부 건강 관리", "산모 신생아 건강관리 지원", "고위험 임산부 의료비 지원", "임신/출산 양육비 및 축하용품 지원"]
    ...
    url = f"{format_url}/subPage.do?menuNo=117013001001"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > div.sub_menu > ul.sub_list > li.active > ul > li > a"):
        if i.text in item_data:
            id_item = i.get("href").split("?menuNo=")[1]
            link_list.append(id_item)
            
    for link in link_list:
        driver.get(f"{format_url}/subPage.do?menuNo={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.tabMenu")
    ...
    for page_id in page_list:
        page_url = f"{format_url}/subPage.do?menuNo={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#con_head > h2"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("#con_bottom > div.research_group > div.manager > ul > li"):
        ...
        for content in soup.select("#content"):
            for tag in content.find_all("div", class_="tabMenu"):
                tag.extract()
                
            for tag in content.find_all("form", id="frm"):
                tag.extract()
