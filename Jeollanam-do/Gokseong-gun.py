...
try:
    ...
    url = f"{base_url}subPage.do?menuNo=102003001000"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb > div.sub_menu > ul.sub_list > li.active > ul.twoDepth > li > a"):
        if "health/" in i.get("href"):
    ...
    for page_id in page_list:
        page_url = f"{base_url}subPage.do?menuNo={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#con_head > h3"):
            data_dict["title"] = title.get_text()
            
        for edit_date in soup.select("div.con_bottom > div.research_group > div.manager > ul > li"):
        ...
        for content in soup.select("#content"):
            for tag in content.find_all("div", class_="con_bottom"):
                tag.extract()
