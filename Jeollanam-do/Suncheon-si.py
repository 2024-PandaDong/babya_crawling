...
try:
    ...
    url = f"{format_url}/busines_01_01.do"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.menu_tab_wrap > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.title_box > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("div.sub_content"):
            ...
            for a in content.find_all("a", href=True):
                ...
                if file_url == "javascript:void(0);" and "saveFile" in a.get("onclick"):
                    if onclick_value and len(onclick_value.split("'")) > 1:
                        type = onclick_value.split("'")[1]
                    else:
                        type = "" 
                        
                    if onclick_value and len(onclick_value.split("'")) > 3:
                        ext = onclick_value.split("'")[3]
                    else:
                        ext = "hwp" 
                    ...
                    for script in content.select("script"):
                        script_str = str(script.get_text().split("location.href")[0].replace("var name =", "return").replace("name =", "return")) + "}"
                        path = str(script.get_text().split('&downPath=')[1].split('&url=')[0])
                        
                        if "(type" in script.get_text():
                            url = str(script.get_text().split('/&url=')[1].split('"+type+')[0])
                            
                        else:
                            url = str(script.get_text().split('/&url=')[1].split('&newNm=')[0])
                        
                    saveFile = js2py.eval_js(script_str)
                    name = saveFile(type)
                    a['href'] = f"{format_url.split('/health')[0]}/w/fileDown.do?ext={ext}&downPath={path}&url={url}{type}&newNm={name}"
            ...
            body_content = f"<body><section class='sub_container'>{content_str}</section></body>"
