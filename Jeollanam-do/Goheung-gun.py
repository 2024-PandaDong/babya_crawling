...
try:
    ...
    url = f"{base_url}contentsView.do?pageId=health172"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#aside > ul > li.active > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for title in soup.select("div.content_wrap > div.content_top > h3"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#font-area"):
        ...
        for edit_date in soup.select("div.content_bot > div.satisfy > ul.charger > li > p.text"):
            editDate = edit_date.get_text().split(" ")[0]
