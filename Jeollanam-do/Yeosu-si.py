...
try:
    ...
    url = f"{base_url}/service/support/childbirth/grant"
    driver.get(url)
    time.sleep(2)
    ...
    for link in link_list:
        driver.get(f"{base_url}/service/support/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.section_wrap")
        
        if elements:
            for element in elements:
                for i in element.select("ul.depth4 > li > a"):
                    id_item = i.get("href").split("support/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/service/support/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.cont_titlebox > h3"):
