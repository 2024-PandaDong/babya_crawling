...
try:
    ...
    url = f"{base_url}/health_service/mother_child/pregnant_women/pregnant"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li.on > ul > li > a"):
        id_item = i.get("href").split("mother_child/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}/health_service/mother_child/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("#content_box > div.group")
        
        if elements:
            for element in elements:
                for i in element.select("ul.ui-corner-all > li > a"):
                    if i.text != "출산예정일계산":
                        id_item = i.get("href").split("mother_child/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/health_service/mother_child/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("div.content_topbox > div.content_title > strong.subject_title"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#content"):
            for tag in content.find_all("div", id="survey"):
                tag.extract()
        ...
        for edit_date in soup.select("#admin_info > dl.admin_info_right > dd"):
            data_dict["editDate"] = edit_date.get_text().replace(".", "-")
