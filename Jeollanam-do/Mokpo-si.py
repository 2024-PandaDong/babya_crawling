...
try:
    ...
    url = f"{base_url}/medical_expense/promote_childbirth/first_meeting"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li.on > ul > li > a"):
        id_item = i.get("href").split("promote_childbirth/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/medical_expense/promote_childbirth/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#SubMenu > ul.snb > li.on > ul > li.on > a"):
