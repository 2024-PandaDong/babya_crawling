...
try:
    ...
    url = f"{base_url}/business/woman_child/pregnant"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li.on > ul > li > a"):
        id_item = i.get("href").split("woman_child/")[1]
        link_list.append(id_item)
        
    for link in link_list:
        driver.get(f"{base_url}/business/woman_child/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
    ...
    for page_id in page_list:
        page_url = f"{base_url}/business/woman_child/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        if elements:
            for element in elements:
                for title in element.select("ul.depth4 > li.on > a"):
                    data_dict["title"] = title.get_text()
        
        else:
            for title in soup.select("#SubMenu > ul.snb > li.on > ul > li.on > a > span"):
                data_dict["title"] = title.get_text()
