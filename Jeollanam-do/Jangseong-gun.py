...
try:
    ...
    url = f"{base_url}/openinfo/health/maternal_child/maternal_child_01"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#four_depth_nav > dd > ul > li > a"):
        id_item = i.get("href").split("maternal_child/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/openinfo/health/maternal_child/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#four_depth_nav > dd > ul > li.on > a > span"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#content"):
            for tag in content.find_all("dl", id="four_depth_nav"):
                tag.extract()
        ...
        for edit_date in soup.select("#page_admin > dd.page_admin2"):
            editDate = edit_date.get_text().replace(" ", "")
