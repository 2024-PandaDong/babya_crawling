...
try:
    ...
    url = f"{base_url}/business/business_08/business_08_02/yeongam.go"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#snb_list > li.on > ul.depth02 > li > a"):
        id_item = i.get("href").split("business_08/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/business/business_08/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#title_box > h3.title"):
        ...
        for edit_date in soup.select("#latest_date"):
