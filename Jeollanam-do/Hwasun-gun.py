...
try:
    ...
    current_list.append("050400000000")

    page_list = set(current_list) - set(collected_list)

    for page_id in page_list:
        page_url = f"{format_url}/contents.do?S=S06&M={page_id}"
        driver.get(page_url)
        time.sleep(2)
        ...
        for title in soup.select("#contents > div.subTitle > h2"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select("#conText"):
        ...
        for edit_date in soup.select("div.satisfyWrap > div.manager > ul.managerR > li > span.num"):
