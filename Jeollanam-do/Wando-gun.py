...
try:
    ...
    url = f"{format_url}/sub.cs?m=65"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li.snbA > a"):
        if i.text == "임신출산지원" or i.text == "출산장려지원":
            parent_element = i.find_parent("li", class_="snbA")
            for j in parent_element.select("ul.snbB > li > a"):
                if "?m=" in j.get("href"):
                    id_item = j.get("href").split("?m=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/sub.cs?m={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#contents_bookmark_0"):
