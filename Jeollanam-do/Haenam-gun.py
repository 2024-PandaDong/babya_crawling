...
try:
    ...
    format_url = base_url.split(".9is?")[0]
    ...
    url = f"{format_url}.9is?contentUid=18e3368f85a01bc3018611bb0d1f1160"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#location > div.inner > ul.snb > li.has_sub > a"):
        if i.text == "임신부 지원":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.has_sub2 > li > a"):
                id_item = j.get("href").split("?contentUid=")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}.9is?contentUid={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
