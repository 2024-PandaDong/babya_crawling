...
try:
    ...
    url = f"{format_url}/business/childbirth/pregnancy_preparation"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#SubMenu > ul.snb > li > a > span"):
        if i.text == "모자보건사업":
            parent_element = i.find_parent("li")
            for j in parent_element.select("ul.snbB > li > a"):
                id_item = j.get("href").split("childbirth/")[1]
    ...
    for page_id in page_list:
        page_url = f"{format_url}/business/childbirth/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#page_title"):
