...
try:
    ...
    url = f"{base_url}/dept_info/health/health_13/page.wscms"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#four_depth_nav > dd > ul > li > a"):
        id_item = i.get("href").split("health_13/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}/dept_info/health/health_13/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#four_depth_nav > dd > ul > li > a"):
            if page_id in title.get("href"):
                for title_text in title.select("span"):
                    data_dict["title"] = title_text.get_text()
        
        for content in soup.select("#subContent"):
