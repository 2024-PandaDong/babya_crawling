...
try:
    ...
    url = f"{base_url}contentsView.do?pageId=health38"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("aside.left_menu > ul > li.active > ul > li > a"):
    ...
    for page_id in page_list:
        ...
        for edit_date in soup.select("div.satisfaction > div.charger > dl > dd"):
