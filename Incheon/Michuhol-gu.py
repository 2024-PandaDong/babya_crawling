...
try:
    ...
    url = f"{base_url}content.do?sq=520"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("div.subl-menu > ul > li > a"):
        if i.text == "모자":
            parent_div = i.find_parent('li')
            for j in parent_div.select("ul > li > a"):
                id_item = j.get("href").split("?sq=")[1]
                link_list.append(id_item)
    
    for link in link_list:
        driver.get(f"{base_url}content.do?sq={link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        elements = soup.select("div.subr-tab-list")
        
        if elements:
            for element in elements:
                for i in element.select("div.tab-list-l1 > ul.tab-list-s1 > li > a"):
                    id_item = i.get("href").split("?sq=")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}content.do?sq={page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for link in soup.select("html > head > link"):
            if link.get("rel")[0] == "stylesheet":
                ...
                if "sub" not in link_url:
                    link["href"] = urllib.parse.urljoin(base_url, link_url)
                    styles.append(str(link))
                    
        for title in soup.select("div.subr-title-area > h2.subr-title"):
            data_dict["title"] = title.get_text()
            
        for content in soup.select("div.subr-real-content"):
        ...
        for edit_date in soup.select("div.mng-info > ul.mng-info-list > li.date"):
