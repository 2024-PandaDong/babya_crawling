...
try:
    ...
    url = f"{base_url}business/mathernal_child/pregnancy.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')

    for i in soup.select("#leftmenu > ul.mb_hidden > li.sublink.select > ul > li > a"):
    ...
    for page_id in page_list:
        page_url = f"{base_url}business/mathernal_child/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for link in soup.select("html > head > link"):
            if link.get("rel")[0] == "stylesheet":
                ...
                if "clinic/css/common" not in link_url:
                    link["href"] = urllib.parse.urljoin(base_url, link_url)
                    styles.append(str(link))
                
        for title in soup.select("#contents > div.sub_title > div > h3"):
