...
try:
    ...
    url = f"{base_url}business/mother.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("ul.tgltab > li > a"):
        id_item = i.get("href")
    ...
    for page_id in page_list:
        page_url = f"{base_url}business/mother.jsp{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = []
        new_page_id = re.sub(r'(\d+)', lambda x: str(int(x.group(0)) + 1), page_id)
        start_tag = soup.select_one(page_id)
        end_tag = soup.select_one(new_page_id)
        ...
        for title in soup.select(f"{page_id} > h4"):
            data_dict["title"] = title.get_text()
        
        for content in soup.select(page_id):
            content_add = "<div>\n"
            ...
            content_add += re.sub(r'[\s\u00A0-\u00FF]+', " ", str(content))
            
            for subContent in start_tag.find_next_siblings():
                top_div = soup.find('div', id='data_control')
                if subContent == end_tag or subContent == top_div:
                    break
                
                content_add += re.sub(r'[\s\u00A0-\u00FF]+', " ", str(subContent))
