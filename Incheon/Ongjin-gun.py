...
try:
    ...
    url = f"{base_url}business/pregnant.jsp"
    driver.get(url)
    time.sleep(2)
    ...
    for page_id in page_list:
        page_url = f"{base_url}business/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        for title in soup.select("#sub_title > h3"):
        ...
        for edit_date in soup.select("#data_control > div.control > ul.con > li > span.a"):
