...
try:
    ...
    url = f"{base_url}business/child/lifetime.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("#lmenu > ul > li.on > ul > li"):
        elements = i.select("ul.depth4")
        if elements:
            for element in elements:
                for j in element.select("ul.depth4 > li > a"):
                    id_item = j.get("href").split("child/")[1]
    ...
    for page_id in page_list:
        page_url = f"{base_url}business/child/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        styles = []
        link_data = ['<link href="https://www.namdong.go.kr/share/css/common.css"rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/share/css/inc.css" rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/share/css/program.css?v1.1" rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/share/css/sub.css?ver1" rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/share/css/vki.css" rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/clinic/css/inc.css" rel="stylesheet" />',
                     '<link href="https://www.namdong.go.kr/clinic/css/sub.css" rel="stylesheet" />']
        elements = soup.select("#lmenu > ul > li.on > ul > li.on > ul.depth4")
        ...
        if elements:
            for element in elements:
                for subTitle in element.select("ul.depth4 > li.on > a"):
                    data_dict["title"] = subTitle.get_text()
                    
        else:
            for title in soup.select("#con_tit > h3"):
