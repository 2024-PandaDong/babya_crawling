...
try:
    ...
    url_data = [f"{format_url}/hebz0502c", f"{format_url}/hebz0720c"]
    ...
    for url in url_data:
        ...
        for i in soup.select("#lnbWrap > ul.lnb > li.active > ul > li > a"):
            id_item = i.get("onclick").split("'/")[1].split("'")[0]
    ...
    for page_id in page_list:
        ...
        for link in soup.select("html > head > link"):
            if link.get("rel")[0] == "stylesheet":
                ...
                if "base.css" not in link_url:
                    link["href"] = urllib.parse.urljoin(base_url, link_url)
                    styles.append(str(link))

        for title in soup.select("div.sub-contentWrap > h3.content-title"):
            data_dict["title"] = title.get_text().strip()
            
        for edit_date in soup.select("section.bottomCont > div.average-wrap > div.average-head > dl.average-head-title.right > dd"):
            data_dict["editDate"] = edit_date.get_text()
            
        for content in soup.select("div.sub-contentWrap-inner"):
            for tag in content.find_all("section", class_="bottomCont"):
                tag.extract()
