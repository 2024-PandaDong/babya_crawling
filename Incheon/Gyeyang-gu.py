...
try:
    ...
    url = f"{base_url}business/mother/reserve.jsp"
    driver.get(url)
    time.sleep(2)
    soup = BeautifulSoup(driver.page_source, 'html.parser')
    
    for i in soup.select("ul.mb_hidden > li.lm3.select > ul.open > li > a"):
        id_item = i.get("href").split("mother/")[1]
        link_list.append(id_item)
    
    for link in link_list:
        driver.get(f"{base_url}business/mother/{link}")
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
    ...
    for page_id in page_list:
        page_url = f"{base_url}business/mother/{page_id}"
        driver.get(page_url)
        time.sleep(2)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        if elements:
            for element in elements:
                for subTitle in element.select("#tab > ul > li.select > a"):
